...
**	20081011	1.0	Khiem Phan	add another Rho. We now have 3 Rho:
**							1./ Rho = Feynman precription
**							2./ Rho1 = appear in Beta & Phi terms
**							2./ Rho2 = appear in ThetaG term (analytics)
**	20080930	1.0	Quan Phan	eta_beta and phi = 0 when z's are real
**	20080927	1.0	Khiem Phan	Update LnGPositive/Negative: Ln(T1) - Ln(T2) instead of '+'
**							why step (0) = 1/2
**							A0 = B0 = C0 = 1e-10 !!!!(fn_ThetaG)
**	notes: Extract all steps in calculating OmegaPlus & Minus
**	20080920	1.0	Quan Phan	Change my_csgn(Im variable)
**	20080828	1.0	Quan Phan	Fixing T1, T2
**	20080826	1.0	Quan Phan	Fixing a lot of errors, too bad
**	20080819	1.0	Quan Phan	Add input using P (same as LoopTool)
**	20080811	1.0	Quan Phan	return to use Imag_Part for whole equation
**	20080804	1.0	Quan Phan	Test Case 1: Export Result File (numeric)
**						change function fn_f
**	20080804	1.0	Khiem Phan	Print p for input of LoopTools!
**	20080728	1.0	Quan Phan	Change "if" functions to use REAL_PART
**						reedit the screen printing, k, l, m, n = (1,4) not (0,3)
**	20080726	1.0	Khiem Phan	Check code
**	20080717	1.0	Quan Phan	1./ Alter fn_alpha behaviour to bypass
**						 the case AC_lk=0.
**						2./ Edit fn_ThetaG, using Rho2 as omega
**	20080715	1.0	Quan Phan	Add fn_eta_beta and fn_eta_phi
**	20080531	1.0	Quan Phan	Mimic Prof. Son's Coding & Naming
**						 style.
**	20080524	1.0	Quan Phan	Create this file
*******************************************************************************/
...
void input_option(int argc, char *argv[], lst &q, lst &m, ex &Rho, ex &Rho1, ex &Rho2){
	...
	mat_msquare[0]=m1;	mat_msquare[1]=m2;	mat_msquare[2]=m3;	mat_msquare[3]=m4;
	mat_q[0][0]=q10;mat_q[0][1]=0;mat_q[0][2]=0;mat_q[0][3]=0;
	mat_q[1][0]=q20;mat_q[1][1]=q21;mat_q[1][2]=0;mat_q[1][3]=0;
	mat_q[2][0]=q30;mat_q[2][1]=q31;mat_q[2][2]=q32;mat_q[2][3]=0;
...
ex fn_z1phi_im (int m, int l, int k){
	 ...
	 **                                                  2.P_mlk.phi_mlk
	  **/
	ex denom = mat_P[m][l][k]*mat_phi[m][l][k] * 2;
	// we write numerator in tedddgrms of: x + sqrt(x^2 + y + I*z))
	// then the result will be:
	//  imag_part(x)+sqrt(abs(y+I*z+pow(x,2)))*sin(atan2(2*imag_part(x)*real_part(x)+real_part(z)+imag_part(y),-imag_part(z)+real_part(y)-pow(imag_part(x),2)+pow(real_part(x),2))/2)
	//=
	//imag_part(x)+sqrt(abs(y+I*z+pow(x,2)))*sin(atan2(2*imag_part(x)*real_part(x)+z,y-pow(imag_part(x),2)+pow(real_part(x),2))/2)
	ex x, y, z, x_im, x_re;
...
//	if (is_a<numeric>(mat_z1phi[m][l][k]))
		z1phi_mlk_im = imag_part(mat_z1phi[m][l][k]);
//	else{
//		x = mat_E[m][l][k] - mat_Q[m][l][k]*mat_phi[m][l][k];
//		x_im = mat_E_im[m][l][k] - mat_Q_im[m][l][k]*mat_phi[m][l][k];
//		x_re = mat_E_re[m][l][k] - mat_Q_re[m][l][k]*mat_phi[m][l][k];
//		y = -4*mat_P[m][l][k]*mat_phi[m][l][k]*mat_msquare[k];
//		z = -4*mat_P[m][l][k]*mat_phi[m][l][k]*Rho;
//		
//		
//		z1phi_mlk_im = x_im + sqrt(abs(y+I*z+pow(x,2))) * sin(
//			atan2(
//			       x_im*x_re+z * 2,
//			       y-pow(x_im,2)+pow(x_re,2)
//			     )/2
//			);
//		
//		z1phi_mlk_im /= denom;
...
ex fn_z2phi_im (int m, int l, int k){
	...
	// we write numerator in terms of: x - sqrt(x^2 + y + I*z))
	// then the result will be:
	///  imag_part(x)-sqrt(abs(y+I*z+pow(x,2)))*sin(atan2(2*imag_part(x)*real_part(x)+real_part(z)+imag_part(y),-imag_part(z)+real_part(y)-pow(imag_part(x),2)+pow(real_part(x),2))/2)
	//=
	///imag_part(x)-sqrt(abs(y+I*z+pow(x,2)))*sin(atan2(2*imag_part(x)*real_part(x)+z,y-pow(imag_part(x),2)+pow(real_part(x),2))/2)
//	if (is_a<numeric>(mat_z2phi[m][l][k]))
...
//		x_im = mat_E_im[m][l][k] - mat_Q_im[m][l][k]*mat_phi[m][l][k];
//		x_re = mat_E_re[m][l][k] - mat_Q_re[m][l][k]*mat_phi[m][l][k];
//		y = -mat_P[m][l][k]*mat_phi[m][l][k]*mat_msquare[k] * 4;
//		z = -mat_P[m][l][k]*mat_phi[m][l][k]*Rho * 4;
//		
//		
//		z2phi_mlk_im = x_im - sqrt(abs(y+I*z+pow(x,2))) * sin(
//			atan2(
//			       x_im*x_re*2+z,
//			       y-pow(x_im,2)+pow(x_re,2)
//			     )/2
...
ex fn_z1beta_im (int m, int l, int k){
	...
	//=
	///imag_part(x)+sqrt(abs(y+I*z+pow(x,2)))*sin(atan2(2*imag_part(x)*real_part(x)+z,y-pow(imag_part(x),2)+pow(real_part(x),2))/2)
//	if (is_a<numeric>(mat_z1beta[m][l][k]))
		z1beta_mlk_im = imag_part(mat_z1beta[m][l][k]);
//	else{
//		ex x, y, z, x_im, x_re;
//		
//		x = mat_E[m][l][k] - mat_Q[m][l][k]/mat_beta[m][l][k];
//		x_im = mat_E_im[m][l][k] - mat_Q_im[m][l][k]/mat_beta[m][l][k];
//		x_re = mat_E_re[m][l][k] - mat_Q_re[m][l][k]/mat_beta[m][l][k];
//		y = -mat_P[m][l][k]*mat_phi[m][l][k]*mat_msquare[k] * 4;
//		z = -mat_P[m][l][k]*mat_phi[m][l][k]*Rho * 4;
//		
//		z1beta_mlk_im = x_im + sqrt(abs(y+I*z+pow(x,2))) * sin(
//			atan2(
//			       x_im*x_re * 2+z,
//			       y-pow(x_im,2)+pow(x_re,2)
//			     )/2
...
ex fn_z2beta_im (int m, int l, int k){
	...
	//=
	///imag_part(x)+sqrt(abs(y+I*z+pow(x,2)))*sin(atan2(2*imag_part(x)*real_part(x)+z,y-pow(imag_part(x),2)+pow(real_part(x),2))/2)
//	if (is_a<numeric>(mat_z2beta[m][l][k]))
...
//		z = -mat_P[m][l][k]*mat_phi[m][l][k]*Rho * 4;
//		
//		z2beta_mlk_im = x_im - sqrt(abs(y+I*z+pow(x,2))) * sin(
//			atan2(
//			       x_im*x_re * 2  +z,
//			       y-pow(x_im,2)+pow(x_re,2)
//			     )/2
...
//		term2_1_2 = 0, term2_3_4 = 0, term2_5_11 = 0, term2_6_12 = 0, term2_7_13 = 0, term2_8_14 = 0, term2_9_15 = 0, term2_10_16 = 0;
	...
	for(k = 0; k<4; k++) for(l = 0; l<4; l++) if (l!=k) for(m = 0; m<4; m++) if(m!=l && m!=k) for(n = 0; n<4; n++) if(n!=m && n!=l && n!=k){
		...
		cout << "term2_5_7_1= " << term2_5_7_1.evalf() << endl;
		cout << "term2_5_7_2 = " << term2_5_7_2.evalf() << endl;
		cout << "term2_5_7 = " << term2_5_7.evalf() << endl;
		cout << "term2_6_8 = " << term2_6_8 << endl;
		cout << "term2_11_13 = " << term2_11_13 << endl;
		cout << "term2_12_14 = " << term2_12_14 << endl;
		cout << "term2_9_15 = " << term2_9_15 << endl;
		cout << "term2_10_16 = " << term2_10_16 << endl;
#endif
		D0 += term1 * term2;
	}
